...
<circle cx="60" cy="60" r="20" fill="#0a0a0f" opacity="0.12"/>`;
  ...
  function pickColors(style, rand) {
    ...
    return {
      from: mixHex(base, "#ffffff", 0.1 + rand() * 0.12),
      to: mixHex(secondary, "#000000", 0.18 + rand() * 0.18),
      accent: mixHex(tertiary, "#ffffff", 0.18 + rand() * 0.16),
    };
  }
  ...
  function generateGameArt(game) {
    ...
    const colors = pickColors(style, rand);
